...
const partnerTypes = [
  ...
  "Industry Partner & Resource Provider",
  "Media & Outreach Partner",
  "Other Collaboration",
];

export default function PartnerRegisterForm({ onClose }) {
  ...
  const validate = () => {
    ...
    if (!formData.proposal.trim()) newErrors.proposal = "Proposal details are required";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSubmitError(err.message || "Failed to submit partnership proposal. Please try again.");
    } finally {
        ...
        <AnimatePresence mode="wait">
          {!submitSuccess ? (
            ...
            >
              <div>
                ...
                <p className="mt-2 text-sm text-white/50">
                  Establish a partnership, sponsor hackathons, or host collaborative events with IEDC SB College.
                </p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="mt-6 space-y-4">
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className="space-y-1.5">
                    ...
                    />
                    {errors.organizationName && <p className="text-xs text-red-400">{errors.organizationName}</p>}
                  </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    className="w-full rounded-xl border border-white/10 bg-[#0d0d0d] px-4 py-3 text-sm text-white outline-none transition focus:border-[var(--accent)]"
                  >
                    <option value="" disabled>Select partnership profile...</option>
                    {partnerTypes.map((type) => (
                      <option key={type} value={type}>{type}</option>
                    ))}
                  </select>
                  {errors.partnershipType && <p className="text-xs text-red-400">{errors.partnershipType}</p>}
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    rows="3.5"
                    placeholder="Provide details on how you want to partner or support the IEDC community..."
                  ...
                  />
                  {errors.proposal && <p className="text-xs text-red-400">{errors.proposal}</p>}
                </div>
            ...
            >
              ...
              <h2 className="mt-6 text-2xl font-semibold text-white">Proposal Received!</h2>
              <p className="mt-3 max-w-sm text-sm text-white/50 leading-relaxed">
                Thank you for your interest in partnering with IEDC SB College. A member of our external relations team will reach out to you via the provided email to coordinate further!
              </p>
